...
#let highlight-box(body, title: none, colors: none) = context {
  ...
  block(
    ...
    stroke: (
      ...
      rest: 0.5pt + get-color-from(c, "accent-primary"),
    ),
...
#let alert(body, colors: none, ..args) = context {
  let c = resolve-theme-colors(colors)
  text(fill: get-color-from(c, "accent-primary"), weight: "medium", ..args)[#body]
}

// Muted text helper
#let muted(body, colors: none, ..args) = context {
  let c = resolve-theme-colors(colors)
  text(fill: get-color-from(c, "text-muted"), size: size-caption, ..args)[#body]
}

// Subtle text
#let subtle(body, colors: none, ..args) = context {
  let c = resolve-theme-colors(colors)
  text(fill: get-color-from(c, "text-light"), size: size-small, ..args)[#body]
}
